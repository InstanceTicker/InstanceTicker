...
// アイ・コンパス・ハイフン
// 10進数指定用（数値のみで指定できるやつ）
//
// オープンソース用に作成。
//
// 「ニュースサイトの記事URLってなんで /articles/-/番号 って構成が多いのか？」
// と言う疑問から生まれたやつなんですが、10進数だと4桁使うのでオススメしません。
...
if($ext1[1]=='webp' || $ext1[1]=='' && preg_match("/^[0-9]+$/", $ext2[1]) && $ext2[1] > 0){// 数字判定
    // SQLite
    $sql = 'SELECT * FROM `instances` WHERE id = '.$ext2[1].' ';
    $res = $db->query($sql);
