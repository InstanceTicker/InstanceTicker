...
//        $result .= 'vertical-align:var(--fv);width:var(--fw);line-height:var(--fl);';
   ...
   switch($ext1[1]){//拡張子
        ...
        case 'php':
            ...
            $htmh.= '<p>Mod:</p>';
            $htmh.= '<select onchange="location.href=this.options[this.selectedIndex].value">';
            $htmh.= '<option value="'.$srv2.'/330/'.$types.'.'.$ext1[1].'" selected>mod330 (For Mastodon v.3.x.x)</option>';
            $htmh.= '<option value="'.$srv2.'/402/'.$types.'.'.$ext1[1].'">mod402 (For Mastodon v.4.x.x)</option>';
            $htmh.= '</select>';
        ...
        case 'user':
            ...
            $con  = '// ==UserScript=='."\n";
            $con .= '// @name         #InstanceTicker_userjs (Type-'.$types.')'."\n";
            $con .= '// @namespace    '.$srv2.'/'."\n";
            $con .= '// @version      '.$version.'_js'."\n";
        ...
        if($ext1[1] == 'css' || empty($ext1[1])){
            ...
            $htmh.= '<option value="/'.$ext2[0].'/'.$types.'.htm" selected>Usage</option>';
            $htmh.= '</select>';
            
            $htmh.= '<p>Mod:</p>';
            $htmh.= '<select onchange="location.href=this.options[this.selectedIndex].value">';
            $htmh.= '<option value="'.$srv2.'/330/'.$types.'.'.$ext1[1].'" selected>330 (For Mastodon v.3.x.x)</option>';
            $htmh.= '<option value="'.$srv2.'/402/'.$types.'.'.$ext1[1].'">402 (For Mastodon v.4.x.x)</option>';
            $htmh.= '</select>';
            
            $htmh.= '<p>Type:</p>';
            $htmh.= '<select onchange="location.href=this.options[this.selectedIndex].value">';
            $htmh.= '<option value="'.$srv2.'/'.$ext2[0].'/0.htm"'; if($types=='0'){$htmh.='selected';} $htmh.='>Type-0 (SNS LOGO)</option>';
            $htmh.= '<option value="'.$srv2.'/'.$ext2[0].'/1.htm"'; if($types=='1'){$htmh.='selected';} $htmh.='>Type-1 (Favicon )</option>';
            $htmh.= '<option value="'.$srv2.'/'.$ext2[0].'/2.htm"'; if($types=='2'){$htmh.='selected';} $htmh.='>Type-2 (Favicon + BG White)</option>';
            $htmh.= '<option value="'.$srv2.'/'.$ext2[0].'/3.htm"'; if($types=='3'){$htmh.='selected';} $htmh.='>Type-3 (Favicon + BG Black)</option>';
            $htmh.= '</select>';
            $htmh.= '</div>';
