...
- まったく無関係の外部サイトの画像を link rel="icon" で設定するお行儀の悪い サーバーが数件ありました
  - 外部Wikiサイト画像の直リンをするという信じられない行為をしているサーバーも中にはいました（2022年某日当時の話）。
  - Misskey ではこれが参照されて、インスタンス情報で表示されていた事例がありました（やけに画像表示が遅いサーバーがあるなと思ったらそれだった）。
- このようなケースについて、疑問はあるし軽蔑もする。
  - このように、無関係の外部サイトの画像を favicon として参照するようなお行儀の悪いサーバーへの対策だと思ってください。
  - このようなケースであっても、14x14 の画像に手動変換し格納していきますが、著作権をないがしろにする意図ではなく、判別のための利用であるため、問題があれば Issue ください。
- サーバー管理者様は、版権画像を Favicon にするのはなるべく避けたほうがいいかも。。。
  - いつの時代も「版権画像の流用は、ナウなヤングにバカウケが実情」なので、注意したところで「老害はさっさとあっち向け」と言われかねないし、どうにもならん。
