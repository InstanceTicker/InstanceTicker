...
### favicon はキャッシュプロキシにするか、Google/Hatena/duckduckgo の favicon ツールをフリーライドすればいいのではないですか？
- それもいい手ですが、Google 側では、近年仕様変更があったため、使わないようにしています。
- Hatena も表示速度がよくないので、使うにしても申し訳ない感じです。
- duckduckgo のFaviconキャッシュは、1000px だろうが 2000px 以上であろうがオリジナルサイズのままキャッシュされます。
- 使わない理由としては、アニメーション favicon にも対応させたかったためで、手動且つ個別で変換させています。
- 稀に、SVG 形式の favicon を設定するインスタンスが存在し、それにも対応させるためには、手動にでもなります。
...
### あるときの Misskey の インスタンス情報 みたく favicon 直リン参照にすれば？
- Favicon 設定が、Mastodon よりも、比較的カジュアルに設定できる Misskey では、Faviconのマナーが微妙だったりする実情が挙げられます。。
- まったく無関係の外部サイトの画像を link rel="icon" で設定するお行儀の悪い サーバーが数件ありました
  - 外部Wikiサイト画像の直リンをFavicon画像に設定するという信じられない行為をしているサーバーも中にはいました（2022年某日当時の話）。
  - Misskey では、このFavicon設定が参照され、インスタンス情報でそのまま表示される仕様だったのですね。
- このような Misskey の仕様を傍から見てて、考えた末での、14px x 14px の画像を作成していく対策なのです。
  - 無関係の外部サイトの画像を favicon として参照するようなお行儀の悪いサーバーへの対策だと思ってください。
  - このようなケースであっても、14px x 14px の画像に手動変換し格納していきますが、著作権をないがしろにする意図ではなく、判別のための利用であるため、問題があれば Issue ください。
- サーバー管理者様は、版権画像を Favicon にするのはなるべく避けたほうがいいかも。。。
  - OGP画像よりも、参照されることが前提の画像ファイルであることから、それを踏まえて設定して欲しいものです。 
  - しかしながらいつの時代も「<b>版権画像の流用は、ナウなヤングにバカウケ</b>」が実情なので、注意したところで「老害はさっさとあっち向け」と言われかねないし、どうにもならん。
    - おいらの若かりし頃を見ているようで、言える立場にもない（MADフラッシュ職人だったしな）。

### 画像データだけ切り離して、別途で管理すれば良いのではないか。
- それはごもっとも。
- 当初はそうだったが、ミスで公開してしまってから、まあいいっかとなり現在に至る。
- 容量は食うし、著作権的に良くないので、将来的にはその方向も視野に入れています。
